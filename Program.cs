    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                // Seed default admin user (configurable, safe to run multiple times).
                {
                    using var scope = app.Services.CreateScope();
                    var config = scope.ServiceProvider.GetRequiredService<IConfiguration>();
                    var userManager = scope.ServiceProvider.GetRequiredService<UserManager<IdentityUser>>();

                    var enabled = config.GetValue("BootstrapAdmin:Enabled", true);
                    var email = config["BootstrapAdmin:Email"];
                    var password = config["BootstrapAdmin:Password"];
                    var requireChange = config.GetValue("BootstrapAdmin:RequireChangeOnFirstLogin", true);
                    ...
                    if (enabled && !string.IsNullOrWhiteSpace(email) && !string.IsNullOrWhiteSpace(password))
                    {
                        ...
                        if (user is null)
                        {
                            ...
                            if (!create.Succeeded)
                            {
                                Log.Warning("Failed to create bootstrap admin user {Email}: {Errors}", email, string.Join(", ", create.Errors.Select(e => e.Code)));
                                user = null;
                            }
                        }
                        else
                        ...
                        if (user is not null)
                        {
                            var inRole = userManager.IsInRoleAsync(user, Authz.Roles.Admin).GetAwaiter().GetResult();
                            if (!inRole)
                            {
                                var addRole = userManager.AddToRoleAsync(user, Authz.Roles.Admin).GetAwaiter().GetResult();
                                if (!addRole.Succeeded)
                                {
                                    Log.Warning("Failed to add bootstrap admin user {Email} to role {Role}: {Errors}", email, Authz.Roles.Admin, string.Join(", ", addRole.Errors.Select(e => e.Code)));
                                }
                            }
